...
import bingmclobby.Timmety.Listeners.CustomRightClick;
...
public class PacketReader {
    ...
    public void uninjext(Player player) {
                ...
                for (EntityPlayer npc : NPC.NPC) {
                    if (npc.getId() == id) {
                        Bukkit.getScheduler().scheduleSyncDelayedTask(Main.getPlugin(Main.class), new Runnable() {
                            @Override
                            public void run() {
                                Bukkit.getPluginManager().callEvent(new CustomRightClick(player, npc, NPC.npcTypes.get(npc.getId())));
                            }
                        }, 0);
                    }
                }
    ...
    private Object getValue(Object instsance ,String name) {
        Object result = null;

        try {
            Field field = instsance.getClass().getDeclaredField(name);

            field.setAccessible(true);
            result =field.get(instsance);
